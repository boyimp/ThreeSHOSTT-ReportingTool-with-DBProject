...
public partial class UI_WorkPeriodReport : System.Web.UI.Page
{
    ...
    private void BindData()
    {
        try
        {
            ...
            string requestType = Request.QueryString["ReportType"];
            if (requestType != null && requestType.Equals("Short"))
            {
                IsShort = true;
            }
            ...
            if (IsShort && TicketManager.GetDataFetchFrom() == "FromStoredProcedure")
                ds = TicketManager.GetReportSP(dStartDate, dEndDate, Ids, OutletId, IsShort);//(dStartDate, dEndDate, Convert.ToInt32(ddlTicketType.SelectedValue), chkOpen.Checked);      
            else
                ds = TicketManager.GetReport(dStartDate, dEndDate, Ids, OutletId, IsShort);//(dStartDate, dEndDate, Convert.ToInt32(ddlTicketType.SelectedValue), chkOpen.Checked);      
            ...
            rptViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/rptWorkPeriodReport.rdlc");
