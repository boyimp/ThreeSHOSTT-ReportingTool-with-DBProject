...
public partial class UI_Tickets : System.Web.UI.Page
{
    ...
    protected void RadGrid1_NeedDataSource1(object sender, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        if (IsPostBack)
        {
            try
            {
                ...
                Datads = TicketManager.GetTickets(fromDate: dStartDate, toDate: dEndDate, TicketType: Convert.ToInt32(ddlTicketType.SelectedValue), isOnlyOpenTicket: chkOpen.Checked, OutletId: outletId);
                RadGrid1.DataSource = Datads;
                //RadGrid1.GroupingSettings.CaseSensitive = false;
                if (Datads != null && Datads.Tables != null && Datads.Tables.Count > 0 && Datads.Tables[0].Rows.Count > 0)
                {
                    //RadGrid1.DataBind();
                    lblWorkPeriod.Text = "Work Period Considered From " + Convert.ToDateTime(Datads.Tables[0].Rows[0]["StartDate"]).ToString("dd MMM yyy hh:mm:ss tt") + " to " + Convert.ToDateTime(Datads.Tables[0].Rows[0]["EndDate"]).ToString("dd MMM yyy hh:mm:ss tt");
                }
                else
    ...
    protected void RadGrid1_ItemCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
    {
        try
        {
            if (e.Item != null && (e.Item.ItemType == Telerik.Web.UI.GridItemType.Item || e.Item.ItemType == Telerik.Web.UI.GridItemType.AlternatingItem))
            {
                ...
                {
                    case "Preview":
                        GetData(ticketID, ddlOutlets.SelectedItem.Text);
                        break;
                    case "InvoicePreview":
                        GetInvoiceData(ticketID, ddlOutlets.SelectedItem.Text);
                        break;
                    default:
                        break;
                }
    ...
    protected void RadGrid1_ColumnCreated(object sender, Telerik.Web.UI.GridColumnCreatedEventArgs e)
    {
        try
        {
            ...
            if ((e.Column.DataType == typeof(decimal) || e.Column.DataType == typeof(int))&& e.Column.UniqueName != "id")
            {
                ((GridBoundColumn)e.Column).FooterText = "Total: ";// +Datads.Tables[0].Compute($@"Sum({e.Column.UniqueName})", string.Empty);
    ...
    private void GetData(int ticketID,string outletName)
    {
        try
        {
    ...
    private void GetInvoiceData(int ticketID,string OutletName)
    {
        try
        {
            DataSet dSet = new DataSet();
            ...
            string ClientDetails = "Client Name : " + (customerEntity == null ? string.Empty : customerEntity.EntityName) + Environment.NewLine +
                                   ...
                                   "Event : " + (eventEntity == null ? string.Empty : eventEntity.EntityName) + Environment.NewLine +
