...
namespace ThreeS.Domain.Models.Inventory
{
    public class WarehouseConsumption : ValueClass
    {
        ...
        private void UpdateFinalCost(Recipe recipe)
        {
            ...
            var xi = CostItems.Select(x => x.PortionId == recipe.Portion.Id);
            var ci = CostItems.FirstOrDefault(x => x.PortionId == recipe.Portion.Id);
            if (ci == null)
            {
                return;
            }
        ...
        private void CreatePeriodicConsumptionItem(InventoryItem inventoryItem, WarehouseConsumption previousWhc, IEnumerable<InventoryTransaction> transactionItems)
        {
            ...
            if (previousWhc != null)
            {
                var previousPci = previousWhc.PeriodicConsumptionItems.FirstOrDefault(x => x.InventoryItemId == inventoryItem.Id);
                if (previousPci != null)
                {
                    pci.InStock =
            ...
            var OutStock = tim.Where(x => x.SourceWarehouseId == WarehouseId).Sum(x => x.Quantity * x.Multiplier) / pci.UnitMultiplier;
            var OutStockValue = tim.Where(x => x.SourceWarehouseId == WarehouseId).Sum(x => x.Quantity * x.Price) ;
            pci.Purchase -= tim.Where(x => x.SourceWarehouseId == WarehouseId).Sum(x => x.Quantity * x.Multiplier) / pci.UnitMultiplier;
            var purchaseAmount = tim.Where(x => x.TargetWarehouseId == WarehouseId).Sum(x => x.Quantity * x.Multiplier) / pci.UnitMultiplier;
            var totalPrice = tim.Sum(x => x.Price * x.Quantity);
            var totalPurchasePrice = tim.Where(x => x.TargetWarehouseId == WarehouseId).Sum(x => x.Quantity * x.Price);
            if (pci.InStock + purchaseAmount  != 0)
            {
                pci.Cost = decimal.Round((totalPurchasePrice + previousCost) / (pci.InStock + purchaseAmount), 2); //decimal.Round((totalPurchasePrice + previousCost + OutStockValue) / (pci.InStock + pci.Purchase + OutStock), 2);
            }
