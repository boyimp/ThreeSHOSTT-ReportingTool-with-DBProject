...
namespace ThreeS.Domain.Models.Tickets
{
    ...
    public class Order : BusinessObjects.ValueClass
    {
        ...
        {
            var cv = OrderCalculationTypeValues.FirstOrDefault(x => x.CalculationTypeID == calculationTypeId);
        ...
        {
            if (!(this.DecreaseInventory || this.IncreaseInventory))
            {
                return false;
            }
            ...
            if (maps.FirstOrDefault() == null || !this.CalculatePrice)
            {
                return false;
            }
            else
            {
                ...
                {
                    CalculationTypeValue oCValue = CalculationTypeValues.FirstOrDefault(x => x.ID == calculation.CalculationTypeId);
                    if (oCValue != null)
                    {
                        return true;
                    }
        ...
        public void UpdatePortion(MenuItemPortion portion, string priceTag, IEnumerable<TaxTemplate> taxTemplates)
        {
            ...
            {
                string tag = priceTag;
                var price = portion.Prices.FirstOrDefault(x => x.PriceTag == tag);
        ...
        public bool ToggleOrderTag(OrderTagGroup orderTagGroup, OrderTag orderTag, int userId, string tagNote)
        {
            ...
            if (otag == null)
            {
                ...
                {
                    var sTag = OrderTagValues.FirstOrDefault(x => x.OrderTagGroupId == orderTag.OrderTagGroupId);
                    if (sTag != null)
                    {
        ...
        {
            return OrderStateValues.FirstOrDefault(x => x.StateName == groupName) ?? OrderStateValue.Default;
        }

        public void SetStateValue(string groupName, int groupOrder, string state, int stateOrder, string stateValue)
        {
            var sv = OrderStateValues.FirstOrDefault(x => x.StateName == groupName);
            if (sv == null)
            {
        ...
        {
            if (_ticketSum == 0 || _totalAmount == 0)
            {
                return 0;
            }
        ...
        public TaxValue GetTaxValue(string taxTemplateName)
        {
            return TaxValues.Any(x => x.TaxTemplateName == taxTemplateName)
                ? GetTaxValues().FirstOrDefault(x => x.TaxTemplateName == taxTemplateName)
                : TaxValue.Empty;
        }
