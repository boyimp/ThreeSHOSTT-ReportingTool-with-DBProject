    ...
    public class EntityType : EntityClass, IOrderable
    {
        ...
        public IEnumerable<EntityCustomField> GetMatchingFields(Entity entity, string searchString)
        {
            if (!string.IsNullOrEmpty(entity.CustomData))
            {
                ...
                var data = searchString == null ? entity.CustomData.Split(new[] { "}," }, StringSplitOptions.RemoveEmptyEntries): 
                    entity.CustomData.Split(new[] { "}," }, StringSplitOptions.RemoveEmptyEntries)
                    .Where(x => Regex.Match(x, "\"Value\":\"([^\"]+)\"").Groups[1].Value.ToLower().Contains(searchString.ToLower()));
                return searchString == null ? EntityCustomFields.ToList().OrderByDescending(x => x.Name) : EntityCustomFields.Where(x => data.Any(y => y.ToLower().Contains(string.Format(nameFormat, x.Name).ToLower()) && y.ToLower().Contains(searchString.ToLower())));
            }
