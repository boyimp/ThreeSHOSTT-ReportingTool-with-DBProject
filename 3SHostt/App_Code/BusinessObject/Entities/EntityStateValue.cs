...
namespace ThreeS.Domain.Models.Entities
{
    public class EntityStateValue : ValueClass
    {
        ...
        {
            var sv= EntityStateValues.FirstOrDefault(x => x.StateName == stateName) ?? EntityStateVal.Default;
            return sv.State;
        }

        public void SetStateValue(string groupName, string state)
        {
            var sv = EntityStateValues.FirstOrDefault(x => x.StateName == groupName);
            if (sv == null)
            {
        ...
        public bool IsInState(string stateName, string state)
        {
            ...
            if (stateName == "*")
            {
                return EntityStateValues.Any(x => x.State == state);
            }

            if (string.IsNullOrEmpty(state))
            {
                return EntityStateValues.All(x => x.StateName != stateName);
            }
