...
CREATE PROC [dbo].[GetMenuItemAccountDetailsForLegacyReportingTool]
(	
	@IsExactTime BIT, 	
	@StartDate DATETIME,
	@EndDate DATETIME,
...
-----------------------------------------------------------------------------------------------------------------
SELECT MenuGroupName GroupCode, MenuItemName, PortionName, Price, 0.00 Gross, 0.00 Quantity, 0.00 Gift, [KEY] SortOrder, Perspective UltimateAccount, Value DistinctAmount , 0.00 TotalCollecton
FROM 
(
	SELECT 
	--TickId, MenuItemName
	'Key' = (SELECT 3 + max(SortOrder) FROM CalculationTypes WHERE Name = CalculationName),
	MenuGroupName, MenuItemName, PortionName, Price, CalculationName Perspective, Sum(CalculationAmount) Value		
	FROM 
	(
		SELECT o.Id AS OrderId, TicketId TickId, MenuItemId, o.MenuGroupName, MenuItemName, o.Total, o.PortionName,
		o.Price Price, o.Quantity
	...
	MenuGroupName, MenuItemName, PortionName, Price, CalculationName
	...
	SELECT 2 'Key', MenuGroupName, MenuItemName, PortionName, Price, 'Sales' Perspective, Sum(Total) Value 
	FROM Orders WHERE TicketId
	IN
	(	
		SELECT id FROM Tickets WHERE [Date] BETWEEN @FDate AND @TDate
	...
	AND orders.CalculatePrice = 1
	GROUP BY MenuGroupName, MenuItemName, PortionName, Price
	...
	SELECT 1 'Key', MenuGroupName, MenuItemName, PortionName, Price, 'Quantity' Perspective, Sum(orders.Quantity)Value 
	FROM Orders WHERE TicketId
	IN
	(	
		SELECT id FROM Tickets WHERE [date] BETWEEN @FDate AND @TDate
	...
	[Key]
	, 
	o.MenuGroupName, o.MenuItemName, o.PortionName, o.Price, p.Name 	Perspective, 
	...
	AND t.TotalAmount > 0
	GROUP BY o.MenuGroupName, o.MenuItemName, o.PortionName, o.Price, p.Name, [Key]	
	...
	o.MenuGroupName, o.MenuItemName, o.PortionName, o.Price, 'Total' Perspective, 
	sum((o.UptoAll/t.TotalAmount)*Amount) Amount
	FROM 
	Tickets t 
	LEFT OUTER JOIN Orders o
	...
	AND t.TotalAmount > 0
	GROUP BY o.MenuGroupName, o.MenuItemName, o.PortionName, o.Price
	--------------------------------TotalPaymentRelation----------------------------------------------------------
	----------------------------------Void Value------------------------------------------------------------------	
	UNION ALL
	...
	MenuGroupName, MenuItemName, PortionName, Price, 'Void' Perspective, Sum(PlainTotal) Value 
	FROM Orders WHERE TicketId
	IN
	(	
		SELECT id FROM Tickets WHERE [Date] BETWEEN @FDate AND @TDate
